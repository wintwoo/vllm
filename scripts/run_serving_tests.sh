...
# BENCH="model_dir,gpu_type,model_identifier,tensor_parallel_size,request_rate_1:request_rate_2:request_rate_3 ..."
#
# Pass to the run_benchmark function using
# run_benchmark $BENCH

L4_7B="$LLAMA2_7B_DIR,l4,7B,1,inf:1:2:3:4
$LLAMA2_7B_DIR,l4,7B,2,inf:1:2:3:4,5,6
$LLAMA2_7B_DIR,l4,7B,4,inf:1:2:3:4,5,6:7:8"

L4_13B="$LLAMA2_13B_DIR,l4,13B,2,inf:1:2:3:4
$LLAMA2_13B_DIR,l4,13B,4,inf:1,2:3:4:5:6
$LLAMA2_13B_DIR,l4,13B,8,inf:1:2:3:4,5,6,7,8"

L4_70B="$LLAMA2_70B_DIR,l4,70B,8,inf:1:2:3:4,5,6,7,8"
...
run_benchmark() {
    while IFS= read -r line; do
        tokens=(${line//,/ })
        echo Model: ${tokens[0]}, GPU type: ${tokens[1]}, Model: ${tokens[2]} TP size: ${tokens[3]}
        # start vllm server
        gcloud compute ssh $VLLM_HOST --zone $ZONE --command \
            "source /etc/profile && screen -L -Logfile /tmp/out.log -d -m python3 -m vllm.entrypoints.api_server --model ${tokens[0]} --disable-log-requests --tensor-parallel-size ${tokens[3]} --swap-space 16 --max-model-len 2048"
        ...
        IFS=':' read -ra request_rates <<< ${tokens[4]}
        for i in "${request_rates[@]}"; do
            echo Running benchmark at $i qps with tp_size ${tokens[3]}
            gcloud compute ssh $VLLM_CLIENT --zone $ZONE --command \
                "source /etc/profile && cd $VLLM_SRC && screen -L -Logfile /tmp/out.log -d -m python3 benchmarks/benchmark_serving.py --backend vllm --host $VLLM_HOST --port $VLLM_PORT --gpu_type ${tokens[1]} --tokenizer $VLLM_TOKENIZER --dataset $VLLM_DATASET --output_dir $OUTPUT_DIR --num-prompts $NUM_PROMPTS --request-rate $i --model ${tokens[2]} --tp_size ${tokens[3]} --fixed_output_length"
            while true; do
